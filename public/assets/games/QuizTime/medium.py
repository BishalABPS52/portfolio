medium_questions = [
    {
        "question": "San Siro is a stadium located in which city?",
        "correct": "Milan",
        "wrong": [
            "Rome",
            "Paris",
            "Madrid",
            "Berlin"
        ]
    },
    {
        "question": "In which city would you find the historic landmark, the Alhambra?",
        "correct": "Granada",
        "wrong": [
            "Seville",
            "Madrid",
            "Barcelona",
            "Lisbon"
        ]
    },
    {
        "question": "Which city is known as the 'City of Canals'?",
        "correct": "Venice",
        "wrong": [
            "Amsterdam",
            "Paris",
            "Bangkok",
            "Venice"
        ]
    },
    {
        "question": "In which city is the famous landmark, the Christ the Redeemer statue, located?",
        "correct": "Rio de Janeiro",
        "wrong": [
            "Sao Paulo",
            "Buenos Aires",
            "Mexico City",
            "Lima"
        ]
    },
    {
        "question": "Which city is home to the historic landmark, the Colosseum?",
        "correct": "Rome",
        "wrong": [
            "Athens",
            "Paris",
            "London",
            "New York"
        ]
    },
    {
        "question": "What is the SI unit of electric current?",
        "correct": "Ampere",
        "wrong": [
            "Volt",
            "Watt",
            "Ohm",
            "Joule"
        ]
    },
    {
        "question": "Who is known as the father of modern physics?",
        "correct": "Albert Einstein",
        "wrong": [
            "Isaac Newton",
            "Niels Bohr",
            "Galileo Galilei",
            "Max Planck"
        ]
    },
    {
        "question": "What is the speed of light in a vacuum?",
        "correct": "approax. 300,000,000 m/s",
        "wrong": [
            "approax. 350,000,000 m/s",
            "150,000,000 m/s",
            "400,000,000 m/s",
            "200,000,000 m/s"
        ]
    },
    {
        "question": "What is the principle stating that energy cannot be created or destroyed?",
        "correct": "Conservation of Energy",
        "wrong": [
            "Newton's First Law",
            "Second Law of Thermodynamics",
            "Law of Inertia",
            "Heisenberg Uncertainty Principle"
        ]
    },
    {
        "question": "Oxalic acid is commonly found in which vegetable?",
        "correct": "Spinach",
        "wrong": [
            "Carrot",
            "Potato",
            "Apple",
            "Lettuce"
        ]
    },
    {
        "question": "Which gas is most commonly used in light bulbs?",
        "correct": "Argon",
        "wrong": [
            "Oxygen",
            "Nitrogen",
            "Helium",
            "Carbon Dioxide"
        ]
    },
    {
        "question": "What is the pH level of Vinegar?",
        "correct": "around 3",
        "wrong": [
            "around 1",
            "exctly 2",
            "around 4",
            "5"
        ]
    },
    {
        "question": "Which element has the chemical symbol 'Na'?",
        "correct": "Sodium",
        "wrong": [
            "Nitrogen",
            "Neon",
            "Nickel",
            "Nobelium"
        ]
    },
    {
        "question": "Who won the Golden Boot at the 2018 FIFA World Cup?",
        "correct": "Harry Kane",
        "wrong": [
            "Cristiano Ronaldo",
            "Lionel Messi",
            "Kylian Mbappe",
            "Eden Hazard"
        ]
    },
    {
        "question": "What does the acronym 'IMF' stand for?",
        "correct": "International Monetary Fund",
        "wrong": [
            "International Management Fund",
            "Internal Monetary Fund",
            "International Market Federation",
            "Investment Money Fund"
        ]
    },
    {
        "question": "Who is the current Secretary-General of the United Nations?",
        "correct": "Ant\u00f3nio Guterres",
        "wrong": [
            "Ban Ki-moon",
            "Kofi Annan",
            "Boutros Boutros-Ghali",
            "Dag Hammarskj\u00f6ld"
        ]
    },
    {
        "question": "Which country is known as the Land of the Midnight Sun?",
        "correct": "Norway",
        "wrong": [
            "Japan",
            "Denmark",
            "Czech",
            "Vietnam"
        ]
    },
    {
        "question": "What is the 2nd largest hot desert in the world?",
        "correct": "Arabian Desert",
        "wrong": [
            "Gobi Desert",
            "Thar Desert",
            "Arctic Desert",
            "Antarctic Desert"
        ]
    },
    {
        "question": "Which planet is known as the Earth's Twins?",
        "correct": "Venus",
        "wrong": [
            "Keplar",
            "Uranus",
            "Neptune",
            "Saturn"
        ]
    },
    {
        "question": "Who wrote the Hamlet?",
        "correct": "William Shakespeare",
        "wrong": [
            "Charles Dickens",
            "J.K. Rowling",
            "George Orwell",
            "Jane Austen"
        ]
    },
    {
        "question": "On a TV screen, the pixels are 1/3 red, 1/3 blue, and 1/3 green. \n If you want to have yellow light, which parts of the pixel have to glow more brightly than the others?",
        "correct": "Red and Green",
        "wrong": [
            "Red and Yellow",
            "Only less Green",
            "Green and Blue",
            "Only less Red"
        ]
    },
    {
        "question": "What is the old name of Japan?",
        "correct": "Nihon/Nippon",
        "wrong": [
            "Yamato",
            "Sakoku",
            "Dait\u014d",
            "Edo"
        ]
    },
    {
        "question": "What is the capital of Japan?",
        "correct": "Tokyo",
        "wrong": [
            "Kyoto",
            "Osaka",
            "Seoul",
            "Beijing"
        ]
    },
    {
        "question": "What is the smallest bone  in human body?",
        "correct": "Stapes",
        "wrong": [
            "Incus",
            "Femur",
            "Ear",
            "anvil"
        ]
    },
    {
        "question": "Octopus has how many hearts?",
        "correct": "3",
        "wrong": [
            "2",
            "1",
            "4",
            "0"
        ]
    },
    {
        "question": "What is the highest waterfall in the world?",
        "correct": "Angel Falls",
        "wrong": [
            "Niagara Falls",
            "Victoria Falls",
            "Iguazu Falls",
            "Yosemite Falls"
        ]
    },
    {
        "question": "Who was the first Emperor of Rome?",
        "correct": "Augustus",
        "wrong": [
            "Julius Caesar",
            "Nero",
            "Caligula",
            "Tiberius"
        ]
    },
    {
        "question": "Which ancient civilization built the Machu Picchu complex?",
        "correct": "Inca Empire",
        "wrong": [
            "Aztec Empire",
            "Mayan Empire",
            "Roman Empire",
            "Persian Empire"
        ]
    },
    {
        "question": "Which treaty ended World War I?",
        "correct": "Treaty of Versailles",
        "wrong": [
            "Treaty of Paris",
            "Treaty of Brest-Litovsk",
            "Treaty of Trianon",
            "Treaty of Ghent"
        ]
    },
    {
        "question": "In what year did the Berlin Wall fall?",
        "correct": "1989",
        "wrong": [
            "1991",
            "1975",
            "1961",
            "2000"
        ]
    },
    {
        "question": "Who was the last Tsar of Russia?",
        "correct": "Nicholas II",
        "wrong": [
            "Alexander III",
            "Peter the Great",
            "Ivan the Terrible",
            "Catherine the Great"
        ]
    },
    {
        "question": "What event marked the beginning of the French Revolution?",
        "correct": "Storming of the Bastille",
        "wrong": [
            "Execution of Louis XVI",
            "Reign of Terror",
            "Tennis Court Oath",
            "Napoleonic Wars"
        ]
    },
    {
        "question": "Who wrote 'The Communist Manifesto'?",
        "correct": "Karl Marx and Friedrich Engels",
        "wrong": [
            "Vladimir Lenin",
            "Joseph Stalin",
            "Adam Smith",
            "Jean-Jacques Rousseau"
        ]
    },
    {
        "question": "Who was the famous queen of Egypt known for her relationships with Julius Caesar and Mark Antony?",
        "correct": "Cleopatra",
        "wrong": [
            "Nefertiti",
            "Hatshepsut",
            "Arsinoe IV",
            "Boudica"
        ]
    },
    {
        "question": "Which battle is considered the turning point of the American Civil War?",
        "correct": "Battle of Gettysburg",
        "wrong": [
            "Battle of Antietam",
            "Battle of Fort Sumter",
            "Battle of Bull Run",
            "Battle of Vicksburg"
        ]
    },
    {
        "question": "Who was the first female pharaoh of Egypt?",
        "correct": "Hatshepsut",
        "wrong": [
            "Cleopatra",
            "Nefertiti",
            "Sobekneferu",
            "Tawosret"
        ]
    },
    {
        "question": "What was the main cause of the Opium Wars?",
        "correct": "British trade of opium in China",
        "wrong": [
            "Religious conflict",
            "Territorial disputes",
            "Naval blockades",
            "Piracy"
        ]
    },
    {
        "question": "Who was the leader of the Soviet Union during World War II?",
        "correct": "Joseph Stalin",
        "wrong": [
            "Vladimir Lenin",
            "Leon Trotsky",
            "Nikita Khrushchev",
            "Mikhail Gorbachev"
        ]
    },
    {
        "question": "What was the name of the ship that sank in 1912 after hitting an iceberg?",
        "correct": "Titanic",
        "wrong": [
            "Lusitania",
            "Britannic",
            "Queen Mary",
            "Endurance"
        ]
    },
    {
        "question": "Which ancient civilization is known for its pyramids and hieroglyphics?",
        "correct": "Ancient Egypt",
        "wrong": [
            "Aztecs",
            "Mayans",
            "Babylonians",
            "Persians"
        ]
    },
    {
        "question": "Who was the first man to circumnavigate the globe?",
        "correct": "Ferdinand Magellan",
        "wrong": [
            "Christopher Columbus",
            "James Cook",
            "Marco Polo",
            "Amerigo Vespucci"
        ]
    },
    {
        "question": "What was the primary religion of the Ottoman Empire?",
        "correct": "Islam",
        "wrong": [
            "Christianity",
            "Judaism",
            "Zoroastrianism",
            "Hinduism"
        ]
    },
    {
        "question": "Who was the famous philosopher known for the quote 'I think, therefore I am'?",
        "correct": "Ren\u00e9 Descartes",
        "wrong": [
            "Plato",
            "Aristotle",
            "Immanuel Kant",
            "David Hume"
        ]
    },
    {
        "question": "Who was the first woman to fly solo across the Atlantic Ocean?",
        "correct": "Amelia Earhart",
        "wrong": [
            "Bessie Coleman",
            "Harriet Quimby",
            "Jacqueline Cochran",
            "Valentina Tereshkova"
        ]
    },
    {
        "question": "What was the name of the treaty that ended the American Revolutionary War?",
        "correct": "Treaty of Paris (1783)",
        "wrong": [
            "Treaty of Ghent",
            "Treaty of Versailles",
            "Treaty of Westphalia",
            "Treaty of Utrecht"
        ]
    },
    {
        "question": "Who was the first female Prime Minister of the United Kingdom?",
        "correct": "Margaret Thatcher",
        "wrong": [
            "Theresa May",
            "Angela Merkel",
            "Indira Gandhi",
            "Benazir Bhutto"
        ]
    },
    {
        "question": "What was the name of the first manned mission to land on the Moon?",
        "correct": "Apollo 11",
        "wrong": [
            "Apollo 13",
            "Sputnik 1",
            "Challenger",
            "Voyager 1"
        ]
    },
    {
        "question": "What was the main cause of World War I?",
        "correct": "Assassination of Archduke Franz Ferdinand",
        "wrong": [
            "Treaty of Versailles",
            "Invasion of Poland",
            "Rise of Nazism",
            "Attack on Pearl Harbor"
        ]
    },
    {
        "question": "What was the name of the first artificial satellite launched into space?",
        "correct": "Sputnik 1",
        "wrong": [
            "Explorer 1",
            "Apollo 11",
            "Vostok 1",
            "Hubble"
        ]
    },
    {
        "question": "What event triggered the start of World War II?",
        "correct": "Germany's invasion of Poland",
        "wrong": [
            "Attack on Pearl Harbor",
            "The Treaty of Versailles",
            "The Great Depression",
            "Rise of Adolf Hitler"
        ]
    },
    {
        "question": "Who was the famous leader of the Indian independence movement?",
        "correct": "Mahatma Gandhi",
        "wrong": [
            "Jawaharlal Nehru",
            "Subhas Chandra Bose",
            "Sardar Patel",
            "Indira Gandhi"
        ]
    },
    {
        "question": "Who was the first female Prime Minister of the United Kingdom?",
        "correct": "Margaret Thatcher",
        "wrong": [
            "Theresa May",
            "Angela Merkel",
            "Indira Gandhi",
            "Benazir Bhutto"
        ]
    },
    {
        "question": "What was the name of the first manned mission to land on the Moon?",
        "correct": "Apollo 11",
        "wrong": [
            "Apollo 13",
            "Sputnik 1",
            "Challenger",
            "Voyager 1"
        ]
    },
    {
        "question": "What was the main cause of World War I?",
        "correct": "Assassination of Archduke Franz Ferdinand",
        "wrong": [
            "Treaty of Versailles",
            "Invasion of Poland",
            "Rise of Nazism",
            "Attack on Pearl Harbor"
        ]
    },
    {
        "question": "What was the name of the first artificial satellite launched into space?",
        "correct": "Sputnik 1",
        "wrong": [
            "Explorer 1",
            "Apollo 11",
            "Vostok 1",
            "Hubble"
        ]
    },
    {
        "question": "What event triggered the start of World War II?",
        "correct": "Germany's invasion of Poland",
        "wrong": [
            "Attack on Pearl Harbor",
            "The Treaty of Versailles",
            "The Great Depression",
            "Rise of Adolf Hitler"
        ]
    },
    {
        "question": "Who was the famous leader of the Indian independence movement?",
        "correct": "Mahatma Gandhi",
        "wrong": [
            "Jawaharlal Nehru",
            "Subhas Chandra Bose",
            "Sardar Patel",
            "Indira Gandhi"
        ]
    },
    {
        "question": "What was the name of the ship that brought the Pilgrims to America?",
        "correct": "Mayflower",
        "wrong": [
            "Santa Maria",
            "HMS Beagle",
            "Titanic",
            "Endeavour"
        ]
    },
    {
        "question": "Who was the first President of the United States to be impeached?",
        "correct": "Andrew Johnson",
        "wrong": [
            "Richard Nixon",
            "Bill Clinton",
            "Donald Trump",
            "Thomas Jefferson"
        ]
    },
    {
        "question": "What was the capital of the Inca Empire?",
        "correct": "Cusco",
        "wrong": [
            "Lima",
            "Quito",
            "Machu Picchu",
            "Bogot\u00e1"
        ]
    },
    {
        "question": "Who was the first person to climb Mount Everest?",
        "correct": "Sir Edmund Hillary",
        "wrong": [
            "Tenzing Norgay",
            "George Mallory",
            "Reinhold Messner",
            "Anatoli Boukreev"
        ]
    },
    {
        "question": "What was the name of the first successful vaccine developed?",
        "correct": "Smallpox vaccine",
        "wrong": [
            "Polio vaccine",
            "Measles vaccine",
            "COVID-19 vaccine",
            "Tetanus vaccine"
        ]
    },
    {
        "question": "Who was the first woman to win a Nobel Prize in Physics?",
        "correct": "Marie Curie",
        "wrong": [
            "Lise Meitner",
            "Rosalind Franklin",
            "Dorothy Crowfoot Hodgkin",
            "Chien-Shiung Wu"
        ]
    },
    {
        "question": "What was the name of the treaty that ended the Vietnam War?",
        "correct": "Paris Peace Accords",
        "wrong": [
            "Treaty of Versailles",
            "Geneva Accords",
            "Treaty of Ghent",
            "Yalta Agreement"
        ]
    },
    {
        "question": "Who was the first female astronaut in space?",
        "correct": "Valentina Tereshkova",
        "wrong": [
            "Sally Ride",
            "Eileen Collins",
            "Mae Jemison",
            "Yuri Gagarin"
        ]
    },
    {
        "question": "What was the name of the first manned spacecraft to orbit the Earth?",
        "correct": "Vostok 1",
        "wrong": [
            "Apollo 11",
            "Sputnik 1",
            "Gemini 4",
            "Mercury-Redstone 3"
        ]
    },
    {
        "question": "Who was the first woman to receive a medical degree in the United States?",
        "correct": "Elizabeth Blackwell",
        "wrong": [
            "Florence Nightingale",
            "Clara Barton",
            "Dorothea Dix",
            "Marie Curie"
        ]
    },
    {
        "question": "Who was the famous scientist known for the laws of motion and gravity?",
        "correct": "Isaac Newton",
        "wrong": [
            "Galileo Galilei",
            "Johannes Kepler",
            "Aristotle",
            "Albert Einstein"
        ]
    },
    {
        "question": "Who was the famous leader of the French Revolution?",
        "correct": "Maximilien Robespierre",
        "wrong": [
            "Napoleon Bonaparte",
            "Louis XVI",
            "Jean-Paul Marat",
            "Georges Danton"
        ]
    },
    {
        "question": "Who was the famous leader of the American Revolution?",
        "correct": "George Washington",
        "wrong": [
            "Thomas Jefferson",
            "Alexander Hamilton",
            "John Adams",
            "Benjamin Franklin"
        ]
    },
    {
        "question": "Who was the famous leader of the Russian Revolution?",
        "correct": "Vladimir Lenin",
        "wrong": [
            "Joseph Stalin",
            "Leon Trotsky",
            "Nicholas II",
            "Mikhail Gorbachev"
        ]
    },
    {
        "question": "Who discovered the law of electromagnetic induction?",
        "correct": "Michael Faraday",
        "wrong": [
            "James Clerk Maxwell",
            "Nikola Tesla",
            "Andr\u00e9-Marie Amp\u00e8re",
            "Isaac Newton"
        ]
    },
    {
        "question": "Who invented dynamite and founded the Nobel Prizes?",
        "correct": "Alfred Nobel",
        "wrong": [
            "Marie Curie",
            "Thomas Edison",
            "Alexander Graham Bell",
            "Robert Koch"
        ]
    },
    {
        "question": "What device did Thomas Edison patent in 1879?",
        "correct": "Light bulb",
        "wrong": [
            "Phonograph",
            "Motion picture camera",
            "Radio",
            "Electric motor"
        ]
    },
    {
        "question": "Who invented the World Wide Web?",
        "correct": "Tim Berners-Lee",
        "wrong": [
            "Steve Jobs",
            "Bill Gates",
            "Vint Cerf",
            "Alan Turing"
        ]
    },
    {
        "question": "What groundbreaking discovery did Marie Curie make?",
        "correct": "Radioactivity",
        "wrong": [
            "X-rays",
            "Theory of relativity",
            "DNA structure",
            "Quantum mechanics"
        ]
    },
    {
        "question": "What is the only metal that is liquid at room temperature?",
        "correct": "Mercury",
        "wrong": [
            "Gallium",
            "Cesium",
            "Lead",
            "Bismuth"
        ]
    },
    {
        "question": "Which animal can rotate its head 270 degrees?",
        "correct": "Owl",
        "wrong": [
            "Chameleon",
            "Eagle",
            "Bat",
            "Praying mantis"
        ]
    },
    {
        "question": "What is the largest living organism on Earth?",
        "correct": "Honey fungus",
        "wrong": [
            "Blue whale",
            "Sequoia tree",
            "Coral reef",
            "Elephant"
        ]
    },
    {
        "question": "What is the SI unit of force?",
        "correct": "Newton",
        "wrong": [
            "Joule",
            "Watt",
            "Pascal",
            "Volt"
        ]
    },
    {
        "question": "What is the term for the resistance of an object to change in motion?",
        "correct": "Inertia",
        "wrong": [
            "Momentum",
            "Friction",
            "Velocity",
            "Acceleration"
        ]
    },
    {
        "question": "What is the study of sound waves called?",
        "correct": "Acoustics",
        "wrong": [
            "Optics",
            "Electrodynamics",
            "Thermodynamics",
            "Kinematics"
        ]
    },
    {
        "question": "What is the process by which heat transfers through direct contact?",
        "correct": "Conduction",
        "wrong": [
            "Convection",
            "Radiation",
            "Diffusion",
            "Evaporation"
        ]
    },
    {
        "question": "What is the term for the bending of light as it passes through different mediums?",
        "correct": "Refraction",
        "wrong": [
            "Reflection",
            "Diffraction",
            "Dispersion",
            "Scattering"
        ]
    },
    {
        "question": "What is the formula for calculating work done?",
        "correct": "Work = Force \u00d7 Distance",
        "wrong": [
            "Work = Mass \u00d7 Acceleration",
            "Work = Power \u00d7 Time",
            "Work = Energy \u00d7 Time",
            "Work = Force \u00d7 Velocity"
        ]
    },
    {
        "question": "What is the name of the particle that carries the electromagnetic force?",
        "correct": "Photon",
        "wrong": [
            "Electron",
            "Neutron",
            "Proton",
            "Gluon"
        ]
    },
    {
        "question": "What is the chemical symbol for water?",
        "correct": "H\u2082O",
        "wrong": [
            "O\u2082",
            "CO\u2082",
            "H\u2082",
            "OH"
        ]
    },
    {
        "question": "What is the most abundant gas in the Earth's atmosphere?",
        "correct": "Nitrogen",
        "wrong": [
            "Oxygen",
            "Carbon Dioxide",
            "Argon",
            "Hydrogen"
        ]
    },
    {
        "question": "What is the pH scale of lime water?",
        "correct": "Around 12",
        "wrong": [
            "7",
            "5",
            "2",
            "9"
        ]
    },
    {
        "question": "What is the process of splitting an atom called?",
        "correct": "Nuclear fission",
        "wrong": [
            "Nuclear fusion",
            "Radioactive decay",
            "Electrolysis",
            "Ionization"
        ]
    },
    {
        "question": "What is the term for substances that speed up chemical reactions?",
        "correct": "Catalysts",
        "wrong": [
            "Reactants",
            "Enzymes",
            "Isotopes",
            "Oxidizers"
        ]
    },
    {
        "question": "What is the lightest element on the periodic table?",
        "correct": "Hydrogen",
        "wrong": [
            "Helium",
            "Lithium",
            "Oxygen",
            "Carbon"
        ]
    },
    {
        "question": "What is the process of converting a solid directly into a gas called?",
        "correct": "Sublimation",
        "wrong": [
            "Condensation",
            "Evaporation",
            "Deposition",
            "Fusion"
        ]
    },
    {
        "question": "Who co-founded Apple Inc. alongside Steve Jobs?",
        "correct": "Steve Wozniak",
        "wrong": [
            "Bill Gates",
            "Elon Musk",
            "Tim Cook",
            "Mark Zuckerberg"
        ]
    },
    {
        "question": "What does IBM stand for?",
        "correct": "International Business Machines",
        "wrong": [
            "Internet Business Management",
            "Integrated Business Model",
            "Innovative Business Machines",
            "Intelligent Business Management"
        ]
    },
    {
        "question": "Who is known as the 'Father of Artificial Intelligence'?",
        "correct": "John McCarthy",
        "wrong": [
            "Alan Turing",
            "Geoffrey Hinton",
            "Marvin Minsky",
            "Claude Shannon"
        ]
    },
    {
        "question": "What is the full form of LASER?",
        "correct": "Light Amplification by Stimulated Emission of Radiation",
        "wrong": [
            "Light and Sound Energy Resonance",
            "Light Absorption by Spectral Emission of Radiation",
            "Luminous Amplification by Solar Energy Reflection",
            "Laser Application by Scientific Energy Reactions"
        ]
    },
    {
        "question": "Who developed the Linux operating system?",
        "correct": "Linus Torvalds",
        "wrong": [
            "Richard Stallman",
            "Dennis Ritchie",
            "Ken Thompson",
            "Bill Gates"
        ]
    },
    {
        "question": "What does GPS stand for?",
        "correct": "Global Positioning System",
        "wrong": [
            "Geographic Placement System",
            "Global Processing Service",
            "Geolocation and Positioning Satellite",
            "Global Pathway System"
        ]
    },
    {
        "question": "Who is the founder of Tesla Motors?",
        "correct": "Elon Musk",
        "wrong": [
            "Nikola Tesla",
            "Jeff Bezos",
            "Steve Jobs",
            "Henry Ford"
        ]
    },
    {
        "question": "What does UNESCO stand for?",
        "correct": "United Nations Educational, Scientific and Cultural Organization",
        "wrong": [
            "United Nations Economic and Social Organization",
            "Universal Education for Scientific Cooperation Organization",
            "United Environmental and Scientific Cooperation Organization",
            "United Nations Environmental Safety Organization"
        ]
    },
    {
        "question": "What does WHO stand for?",
        "correct": "World Health Organization",
        "wrong": [
            "World Humanitarian Organization",
            "World Health Organization",
            "World Housing Organization",
            "Worldwide Health Organization"
        ]
    },
    {
        "question": "What does JPEG stand for?",
        "correct": "Joint Photographic Experts Group",
        "wrong": [
            "Joint Photo Editing Group",
            "Joint Picture Experts Group",
            "Joint Photography Editing Group",
            "Java Photographic Experts Group"
        ]
    },
    {
        "question": "What does RAM stand for?",
        "correct": "Random Access Memory",
        "wrong": [
            "Read Access Memory",
            "Rapid Access Memory",
            "Run-time Access Memory",
            "Read-Only Access Memory"
        ]
    },
    {
        "question": "What does DNA stand for?",
        "correct": "Deoxyribonucleic Acid",
        "wrong": [
            "Deoxyribonucleic Atom",
            "Deoxy Ribonucleic Acid",
            "Deoxyribose Nucleic Acid",
            "Deoxyribonucleotide Acid"
        ]
    },
    {
        "question": "What does URL stand for?",
        "correct": "Uniform Resource Locator",
        "wrong": [
            "Universal Resource Locator",
            "Unified Resource Locator",
            "Universal Redirect Link",
            "Uniform Redirect Locator"
        ]
    },
    {
        "question": "What does LED stand for?",
        "correct": "Light Emitting Diode",
        "wrong": [
            "Light Electric Diode",
            "Luminous Emitting Diode",
            "Light Emission Diode",
            "Low Energy Diode"
        ]
    },
    {
        "question": "What does SIM stand for?",
        "correct": "Subscriber Identity Module",
        "wrong": [
            "Subscriber Identification Module",
            "System Identity Module",
            "Subscriber Information Module",
            "Service Identification Module"
        ]
    },
    {
        "question": "Wembley Stadium is located in which city?",
        "correct": "London",
        "wrong": [
            "Manchester",
            "Birmingham",
            "Liverpool",
            "Edinburgh"
        ]
    },
    {
        "question": "The Great Wall of China stretches across how many provinces?",
        "correct": "15",
        "wrong": [
            "12",
            "18",
            "10",
            "20"
        ]
    },
    {
        "question": "Which stadium hosted the first FIFA World Cup final?",
        "correct": "Estadio Centenario",
        "wrong": [
            "Maracan\u00e3 Stadium",
            "Wembley Stadium",
            "La Bombonera",
            "Old Trafford"
        ]
    },
    {
        "question": "Where is the Statue of Liberty located?",
        "correct": "New York City",
        "wrong": [
            "Washington D.C.",
            "Los Angeles",
            "Chicago",
            "San Francisco"
        ]
    },
    {
        "question": "Which city is home to the Eiffel Tower?",
        "correct": "Paris",
        "wrong": [
            "London",
            "Rome",
            "Berlin",
            "Madrid"
        ]
    },
    {
        "question": "What is the largest stadium in the world by capacity?",
        "correct": "Rungrado 1st of May Stadium",
        "wrong": [
            "Melbourne Cricket Ground",
            "Camp Nou",
            "Wembley Stadium",
            "Rose Bowl"
        ]
    },
    {
        "question": "Which island is known as the 'Island of Gods'?",
        "correct": "Bali",
        "wrong": [
            "Hawaii",
            "Mauritius",
            "Phuket",
            "Seychelles"
        ]
    },
    {
        "question": "What is the capital of Iceland?",
        "correct": "Reykjavik",
        "wrong": [
            "Oslo",
            "Helsinki",
            "Copenhagen",
            "Stockholm"
        ]
    },
    {
        "question": "Which desert covers most of North Africa?",
        "correct": "Sahara Desert",
        "wrong": [
            "Kalahari Desert",
            "Atacama Desert",
            "Gobi Desert",
            "Sonoran Desert"
        ]
    },
    {
        "question": "What color is associated with mourning in many Asian cultures?",
        "correct": "White",
        "wrong": [
            "Black",
            "Red",
            "Blue",
            "Purple"
        ]
    },
    {
        "question": "Which monument is considered one of the Seven Wonders of the Ancient World?",
        "correct": "Great Pyramid of Giza",
        "wrong": [
            "Colossus of Rhodes",
            "Machu Picchu",
            "The Colosseum",
            "Taj Mahal"
        ]
    },
    {
        "question": "Which festival is known as the 'Festival of Lights'?",
        "correct": "Diwali",
        "wrong": [
            "Christmas",
            "Hanukkah",
            "Lunar New Year",
            "Eid al-Fitr"
        ]
    },
    {
        "question": "Which country is famous for its tulip fields?",
        "correct": "Netherlands",
        "wrong": [
            "France",
            "Belgium",
            "Germany",
            "Denmark"
        ]
    },
    {
        "question": "Which country is known for its Maasai tribe?",
        "correct": "Kenya",
        "wrong": [
            "Tanzania",
            "Uganda",
            "South Africa",
            "Ethiopia"
        ]
    },
    {
        "question": "What is the process of removing salt from seawater called?",
        "correct": "Desalination",
        "wrong": [
            "Evaporation",
            "Filtration",
            "Purification",
            "Distillation"
        ]
    },
    {
        "question": "What type of soil is best for growing rice?",
        "correct": "Clay",
        "wrong": [
            "Sandy",
            "Loamy",
            "Peaty",
            "Saline"
        ]
    },
    {
        "question": "What is the primary nutrient found in fertilizers?",
        "correct": "Nitrogen",
        "wrong": [
            "Phosphorus",
            "Potassium",
            "Calcium",
            "Magnesium"
        ]
    },
    {
        "question": "What is the process of turning milk into cheese called?",
        "correct": "Cheesemaking",
        "wrong": [
            "Fermentation",
            "Pasteurization",
            "Curdling",
            "Distillation"
        ]
    },
    {
        "question": "What is the term for crops grown specifically to improve soil fertility?",
        "correct": "Cover crops",
        "wrong": [
            "Cash crops",
            "Food crops",
            "Fruit crops",
            "Fiber crops"
        ]
    },
    {
        "question": "What is the nickname of the Australian Open?",
        "correct": "The Happy Slam",
        "wrong": [
            "The Grand Slam",
            "The Sunshine Slam",
            "The Down Under Open",
            "The Melbourne Slam"
        ]
    },
    {
        "question": "Which tennis player has won the most Grand Slam titles?",
        "correct": "Rafael Nadal",
        "wrong": [
            "Roger Federer",
            "Novak Djokovic",
            "Pete Sampras",
            "Serena Williams"
        ]
    },
    {
        "question": "Which sport uses a shuttlecock?",
        "correct": "Badminton",
        "wrong": [
            "Tennis",
            "Table Tennis",
            "Squash",
            "Volleyball"
        ]
    },
    {
        "question": "What is the maximum number of players allowed on a basketball court?",
        "correct": "10",
        "wrong": [
            "12",
            "5",
            "8",
            "15"
        ]
    },
    {
        "question": "Which country has won the most Cricket World Cups?",
        "correct": "Australia",
        "wrong": [
            "India",
            "West Indies",
            "England",
            "Pakistan"
        ]
    },
    {
        "question": "What is the distance of a marathon?",
        "correct": "42.195 kilometers",
        "wrong": [
            "40 kilometers",
            "45 kilometers",
            "50 kilometers",
            "35 kilometers"
        ]
    },
    {
        "question": "Which athlete is known as the 'Fastest Man Alive'?",
        "correct": "Usain Bolt",
        "wrong": [
            "Carl Lewis",
            "Michael Johnson",
            "Tyson Gay",
            "Asafa Powell"
        ]
    },
    {
        "question": "If a cube has 6 faces, how many edges does it have?",
        "correct": "12",
        "wrong": [
            "8",
            "6",
            "10",
            "14"
        ]
    },
    {
        "question": "What is the oldest tennis tournament in the world?",
        "correct": "Wimbledon",
        "wrong": [
            "US Open",
            "French Open",
            "Australian Open",
            "Roland Garros"
        ]
    },
    {
        "question": "What does GPT stand for in AI models like GPT-4?",
        "correct": "Generative Pre-trained Transformer",
        "wrong": [
            "Generalized Processing Technique",
            "Generative Programming Tool",
            "Global Processing Technology",
            "Graphical Processing Tool"
        ]
    },
    {
        "question": "Which company developed the AlphaGo AI that defeated a Go champion?",
        "correct": "DeepMind",
        "wrong": [
            "IBM",
            "Google",
            "Microsoft",
            "Tesla"
        ]
    },
    {
        "question": "What is the term for AI systems that mimic human decision-making?",
        "correct": "Cognitive Computing",
        "wrong": [
            "Artificial Intelligence",
            "Machine Learning",
            "Natural Language Processing",
            "Neural Networks"
        ]
    },
    {
        "question": "What is the difference between supervised and unsupervised learning?",
        "correct": "Supervised learning uses labeled data, while unsupervised learning uses unlabeled data.",
        "wrong": [
            "Supervised learning is faster than unsupervised learning.",
            "Unsupervised learning is based on algorithms.",
            "Supervised learning uses reinforcement.",
            "Unsupervised learning uses neural networks."
        ]
    },
    {
        "question": "Which AI model is known for generating photorealistic images from text prompts?",
        "correct": "DALL\u00b7E",
        "wrong": [
            "GPT-3",
            "AlphaGo",
            "BERT",
            "Clara"
        ]
    },
    {
        "question": "What is the primary programming language used in TensorFlow?",
        "correct": "Python",
        "wrong": [
            "Java",
            "C++",
            "Ruby",
            "JavaScript"
        ]
    },
    {
        "question": "What is the term for AI systems that can learn without being explicitly programmed?",
        "correct": "Machine Learning",
        "wrong": [
            "Artificial Intelligence",
            "Deep Learning",
            "Neural Networks",
            "Natural Language Processing"
        ]
    },
    {
        "question": "Which AI system beat world chess champion Garry Kasparov in 1997?",
        "correct": "Deep Blue",
        "wrong": [
            "AlphaGo",
            "Watson",
            "DeepMind",
            "ChessBot"
        ]
    },
    {
        "question": "What is the process of training an AI model on large datasets called?",
        "correct": "Machine Learning",
        "wrong": [
            "Data Mining",
            "Data Analytics",
            "Supervised Learning",
            "Unsupervised Learning"
        ]
    },
    {
        "question": "What does NLP stand for in AI?",
        "correct": "Natural Language Processing",
        "wrong": [
            "Neural Learning Processing",
            "Network Layer Processing",
            "Numerical Language Processing",
            "Nonlinear Logic Processing"
        ]
    },
    {
        "question": "What is the powerhouse of the cell?",
        "correct": "Mitochondria",
        "wrong": [
            "Nucleus",
            "Endoplasmic Reticulum",
            "Chloroplast",
            "Golgi Apparatus"
        ]
    },
    {
        "question": "What is the process by which plants make food using sunlight?",
        "correct": "Photosynthesis",
        "wrong": [
            "Respiration",
            "Transpiration",
            "Germination",
            "Fermentation"
        ]
    },
    {
        "question": "What is the smallest unit of life?",
        "correct": "Cell",
        "wrong": [
            "Atom",
            "Molecule",
            "Organ",
            "Organism"
        ]
    },
    {
        "question": "Which gas do humans exhale during respiration?",
        "correct": "Carbon Dioxide",
        "wrong": [
            "Oxygen",
            "Nitrogen",
            "Methane",
            "Hydrogen"
        ]
    },
    {
        "question": "What is the term for the study of heredity?",
        "correct": "Genetics",
        "wrong": [
            "Zoology",
            "Ecology",
            "Anatomy",
            "Pathology"
        ]
    },
    {
        "question": "Which part of the brain controls balance and coordination?",
        "correct": "Cerebellum",
        "wrong": [
            "Cerebrum",
            "Medulla Oblongata",
            "Thalamus",
            "Hippocampus"
        ]
    },
    {
        "question": "What is the scientific name for the voice box?",
        "correct": "Larynx",
        "wrong": [
            "Trachea",
            "Esophagus",
            "Pharynx",
            "Bronchi"
        ]
    },
    {
        "question": "What is the default file system used by macOS?",
        "correct": "APFS (Apple File System)",
        "wrong": [
            "NTFS",
            "FAT32",
            "HFS+",
            "exFAT"
        ]
    },
    {
        "question": "Which Linux distribution is known for its user-friendly interface?",
        "correct": "Ubuntu",
        "wrong": [
            "Debian",
            "Fedora",
            "Arch Linux",
            "CentOS"
        ]
    },
    {
        "question": "What is the term for a lightweight OS designed for virtualization?",
        "correct": "Hypervisor",
        "wrong": [
            "Docker",
            "VirtualBox",
            "Vagrant",
            "VMware"
        ]
    },
    {
        "question": "Which OS is used in Apple iPhones?",
        "correct": "iOS",
        "wrong": [
            "Android",
            "Windows Phone",
            "Symbian",
            "HarmonyOS"
        ]
    },
    {
        "question": "Which team has won the most UEFA Champions League titles?",
        "correct": "Real Madrid",
        "wrong": [
            "AC Milan",
            "Barcelona",
            "Liverpool",
            "Bayern Munich"
        ]
    },
    {
        "question": "Who is the all-time top scorer in Premier League history?",
        "correct": "Harry Kane",
        "wrong": [
            "Sergio Ag\u00fcero",
            "Wayne Rooney",
            "Thierry Henry",
            "Frank Lampard"
        ]
    },
    {
        "question": "Which player has won the Ballon d'Or the most times?",
        "correct": "Lionel Messi",
        "wrong": [
            "Cristiano Ronaldo",
            "Michel Platini",
            "Johan Cruyff",
            "Ronaldinho"
        ]
    },
    {
        "question": "Which player was named Player of the Tournament in the 2021 ICC T20 World Cup?",
        "correct": "David Warner",
        "wrong": [
            "Shakib Al Hasan",
            "Babar Azam",
            "Mohammad Rizwan",
            "Jos Buttler"
        ]
    },
    {
        "question": "Who captained India to victory in the 1983 Cricket World Cup?",
        "correct": "Kapil Dev",
        "wrong": [
            "Sourav Ganguly",
            "M.S. Dhoni",
            "Sunil Gavaskar",
            "Virat Kohli"
        ]
    },
    {
        "question": "Who scored the most runs in a single edition of the ICC T20 World Cup?",
        "correct": "Virat Kohli",
        "wrong": [
            "Chris Gayle",
            "Shane Watson",
            "Babar Azam",
            "Jos Buttler"
        ]
    },
    {
        "question": "What is the format of an ICC T20 World Cup match?",
        "correct": "Twenty20",
        "wrong": [
            "Test Match",
            "One Day International",
            "Fifty Overs",
            "T10"
        ]
    },
    {
        "question": "Which bowler took the most wickets in the 2007 ICC T20 World Cup?",
        "correct": "Ajantha Mendis",
        "wrong": [
            "Shahid Afridi",
            "Lasith Malinga",
            "Brett Lee",
            "Yuvraj Singh"
        ]
    },
    {
        "question": "Who hit the winning runs in the 2019 ICC Cricket World Cup final?",
        "correct": "Ben Stokes",
        "wrong": [
            "Joe Root",
            "Jos Buttler",
            "Chris Woakes",
            "Eoin Morgan"
        ]
    },
    {
        "question": "What is the primary ingredient in guacamole?",
        "correct": "Avocado",
        "wrong": [
            "Tomato",
            "Lime",
            "Onion",
            "Cucumber"
        ]
    },
    {
        "question": "Which vegetable is known as a superfood due to its high nutrient content?",
        "correct": "Kale",
        "wrong": [
            "Lettuce",
            "Cabbage",
            "Spinach",
            "Broccoli"
        ]
    },
    {
        "question": "What is the main ingredient in coleslaw?",
        "correct": "Cabbage",
        "wrong": [
            "Lettuce",
            "Carrot",
            "Onion",
            "Celery"
        ]
    },
    {
        "question": "Which vegetable is also called a 'courgette'?",
        "correct": "Zucchini",
        "wrong": [
            "Cucumber",
            "Eggplant",
            "Squash",
            "Bell Pepper"
        ]
    },
    {
        "question": "What is the scientific name for the onion family?",
        "correct": "Allium",
        "wrong": [
            "Brassica",
            "Solanum",
            "Cucurbita",
            "Fabaceae"
        ]
    },
    {
        "question": "Which vegetable is used to make chips?",
        "correct": "Potato",
        "wrong": [
            "Carrot",
            "Turnip",
            "Parsnip",
            "Zucchini"
        ]
    },
    {
        "question": "What is the main ingredient in hummus?",
        "correct": "Chickpeas",
        "wrong": [
            "Lentils",
            "Beans",
            "Peas",
            "Corn"
        ]
    },
    {
        "question": "Which vegetable is orange and rich in beta-carotene?",
        "correct": "Carrot",
        "wrong": [
            "Pumpkin",
            "Sweet Potato",
            "Bell Pepper",
            "Mango"
        ]
    },
    {
        "question": "What is the green part of a leek called?",
        "correct": "Leaves",
        "wrong": [
            "Stem",
            "Root",
            "Bulb",
            "Shoot"
        ]
    },
    {
        "question": "Which vegetable is used to make pickles?",
        "correct": "Cucumber",
        "wrong": [
            "Zucchini",
            "Carrot",
            "Radish",
            "Celery"
        ]
    },
    {
        "question": "What is the name of the edible root of the beet plant?",
        "correct": "Beetroot",
        "wrong": [
            "Radish",
            "Turnip",
            "Carrot",
            "Parsnip"
        ]
    },
    {
        "question": "Which fruit is known for its high vitamin C content?",
        "correct": "Orange",
        "wrong": [
            "Apple",
            "Banana",
            "Watermelon",
            "Grapes"
        ]
    },
    {
        "question": "What color is the inside of a kiwi?",
        "correct": "Green",
        "wrong": [
            "Red",
            "Yellow",
            "Purple",
            "Orange"
        ]
    },
    {
        "question": "Which fruit is also called an alligator pear?",
        "correct": "Avocado",
        "wrong": [
            "Papaya",
            "Mango",
            "Pear",
            "Fig"
        ]
    },
    {
        "question": "What is the largest fruit in the world?",
        "correct": "Jackfruit",
        "wrong": [
            "Pumpkin",
            "Watermelon",
            "Coconut",
            "Durian"
        ]
    },
    {
        "question": "Which fruit is often called a 'love apple'?",
        "correct": "Tomato",
        "wrong": [
            "Cherry",
            "Strawberry",
            "Apple",
            "Pomegranate"
        ]
    },
    {
        "question": "What is the main flavor in a pi\u00f1a colada?",
        "correct": "Pineapple",
        "wrong": [
            "Coconut",
            "Banana",
            "Mango",
            "Lemon"
        ]
    },
    {
        "question": "Which fruit grows on a cactus?",
        "correct": "Prickly Pear",
        "wrong": [
            "Mango",
            "Papaya",
            "Kiwi",
            "Guava"
        ]
    },
    {
        "question": "Which fruit is the main ingredient in jam?",
        "correct": "Strawberry",
        "wrong": [
            "Apple",
            "Banana",
            "Orange",
            "Peach"
        ]
    },
    {
        "question": "Who is the founder of Netflix, the streaming service that changed how we watch TV and movies?",
        "correct": "Reed Hastings",
        "wrong": [
            "Jeff Bezos",
            "Elon Musk",
            "Mark Zuckerberg",
            "Larry Page"
        ]
    },
    {
        "question": "Who co-founded Apple Inc. along with Steve Jobs and Ronald Wayne?",
        "correct": "Steve Wozniak",
        "wrong": [
            "Tim Cook",
            "Elon Musk",
            "Bill Gates",
            "Larry Page"
        ]
    },
    {
        "question": "Who is the founder of Zoom, the video conferencing platform that became popular during the pandemic?",
        "correct": "Eric Yuan",
        "wrong": [
            "Mark Zuckerberg",
            "Elon Musk",
            "Jeff Bezos",
            "Bill Gates"
        ]
    },
    {
        "question": "Who founded PayPal, the online payment system, before it was acquired by eBay?",
        "correct": "Max Levchin",
        "wrong": [
            "Elon Musk",
            "Bill Gates",
            "Larry Page",
            "Mark Zuckerberg"
        ]
    },
    {
        "question": "Who is the founder of WhatsApp, the messaging app later acquired by Meta (formerly Facebook)?",
        "correct": "Jan Koum",
        "wrong": [
            "Mark Zuckerberg",
            "Elon Musk",
            "Brian Chesky",
            "Larry Page"
        ]
    },
    {
        "question": "Who founded Snapchat, the multimedia messaging app popular among younger audiences?",
        "correct": "Evan Spiegel",
        "wrong": [
            "Mark Zuckerberg",
            "Jack Dorsey",
            "Bill Gates",
            "Elon Musk"
        ]
    },
    {
        "question": "Who is the founder of Twitter, the social media platform known for short posts called 'tweets'?",
        "correct": "Jack Dorsey",
        "wrong": [
            "Mark Zuckerberg",
            "Elon Musk",
            "Larry Page",
            "Bill Gates"
        ]
    },
    {
        "question": "Who is the founder of Shopify, the e-commerce platform that helps businesses sell online?",
        "correct": "Tobias L\u00fctke",
        "wrong": [
            "Jeff Bezos",
            "Elon Musk",
            "Mark Zuckerberg",
            "Jack Ma"
        ]
    },
    {
        "question": "Who founded Spotify, the music streaming service that offers millions of songs?",
        "correct": "Daniel Ek",
        "wrong": [
            "Reed Hastings",
            "Elon Musk",
            "Mark Zuckerberg",
            "Brian Chesky"
        ]
    },
    {
        "question": "Who is the founder of LinkedIn, the professional networking platform?",
        "correct": "Reid Hoffman",
        "wrong": [
            "Mark Zuckerberg",
            "Bill Gates",
            "Jeff Bezos",
            "Larry Page"
        ]
    },
    {
        "question": "What is the only mammal capable of true flight?",
        "correct": "Bat",
        "wrong": [
            "Bird",
            "Insect",
            "Flying Squirrel",
            "Pterodactyl"
        ]
    },
    {
        "question": "Which animal can sleep for three years straight?",
        "correct": "Snail",
        "wrong": [
            "Bear",
            "Koala",
            "Hibernating Frog",
            "Cat"
        ]
    },
    {
        "question": "What is the loudest animal on Earth (relative to its size)?",
        "correct": "Cicada",
        "wrong": [
            "Whale",
            "Lion",
            "Elephant",
            "Tiger"
        ]
    },
    {
        "question": "Which bird can fly backward?",
        "correct": "Hummingbird",
        "wrong": [
            "Eagle",
            "Pigeon",
            "Sparrow",
            "Parrot"
        ]
    },
    {
        "question": "What animal has the longest lifespan?",
        "correct": "Greenland shark \u2013 up to 500 years!",
        "wrong": [
            "Tortoise","Elephant","Whale","Ostrich"
        ]
    },
    {
        "question": "Who holds the record for the most goals scored in FIFA World Cup history?",
        "correct": "Marta",
        "wrong": ["Cristiano Ronaldo", "Pele", "Miroslav Klose", "Ronaldo Nazário"]
    },
    {
        "question": "Who was the first person to reach the South Pole?",
        "correct": "Roald Amundsen",
        "wrong": ["Robert Falcon Scott", "Edmund Hillary", "Tenzing Norgay", "Christopher Columbus"]
    },
    {
        "question": "What was the primary cause of the American Civil War?",
        "correct": "Slavery",
        "wrong": ["Economic differences", "State rights", "Political conflict", "Westward expansion"]
    },
    {
        "question": "What is the capital of Argentina?",
        "correct": "Buenos Aires",
        "wrong": ["Santiago", "Rio de Janeiro", "Montevideo", "Lima"]
    },
    {
        "question": "What is the smallest country in the world?",
        "correct": "Vatican City",
        "wrong": ["Monaco", "San Marino", "Nauru", "Liechtenstein"]
    },
    {
        "question": "Which planet has the most moons?",
        "correct": "Jupiter",
        "wrong": ["Saturn", "Mars", "Earth", "Uranus"]
    },
    {
        "question": "What is the capital of Chile?",
        "correct": "Santiago",
        "wrong": ["Valparaiso", "Buenos Aires", "Lima", "La Paz"]
    },
    {
        "question": "What is the largest bird in the world?",
        "correct": "Ostrich",
        "wrong": ["Emu", "Penguin", "Albatross", "Condor"]
    },
    {
        "question": "What is the most populous country in the world?",
        "correct": "China",
        "wrong": ["India", "USA", "Indonesia", "Brazil"]
    },
    {
        "question": "What is the fastest land animal?",
        "correct": "Cheetah",
        "wrong": ["Lion", "Leopard", "Jaguar", "Antelope"]
    },
    {
        "question": "What is the tallest mammal in the world?",
        "correct": "Giraffe",
        "wrong": ["Elephant", "Blue whale", "Horse", "Camel"]
    },
    {
        "question": "What is the most spoken language in the world?",
        "correct": "Mandarin Chinese",
        "wrong": ["Spanish", "English", "Hindi", "Arabic"]
    },
    {
        "question": "Which animal has the longest lifespan?",
        "correct": "Greenland Shark",
        "wrong": ["Elephant", "Tortoise", "Bowhead Whale", "Koi Fish"]
    },
    {
        "question": "Who was the first person to circumnavigate the globe by sea?",
        "correct": "Ferdinand Magellan",
        "wrong": ["Christopher Columbus", "Marco Polo", "John Cabot", "Vasco da Gama"]
    },
    {
        "question": "What was the primary language of the Roman Empire?",
        "correct": "Latin",
        "wrong": ["Greek", "Arabic", "Hebrew", "Etruscan"]
    },
    {
        "question": "What is the capital of Fiji?",
        "correct": "Suva",
        "wrong": ["Nadi", "Port Vila", "Apia", "Honiara"]
    },
    {
        "question": "What is the main component of natural gas?",
        "correct": "Methane",
        "wrong": ["Ethane", "Propane", "Butane", "Acetylene"]
    },
    {
        "question": "What is the capital of South Africa?",
        "correct": "Pretoria",
        "wrong": ["Cape Town", "Johannesburg", "Durban", "Bloemfontein"]
    },
    {
        "question": "What is the fastest fish in the ocean?",
        "correct": "Sailfish",
        "wrong": ["Marlin", "Tuna", "Shark", "Swordfish"]
    },
    {
        "question": "What is the capital of Micronesia?",
        "correct": "Palikir",
        "wrong": ["Koror", "Hagåtña", "Majuro", "Suva"]
    },
    {
        "question": "What is the capital of Palau?",
        "correct": "Ngerulmud",
        "wrong": ["Koror", "Suva", "Port Moresby", "Apia"]
    },
    {
        "question": "What is the capital of Tonga?",
        "correct": "Nuku'alofa",
        "wrong": ["Suva", "Apia", "Port Vila", "Port Moresby"]
    },
    {
        "question": "What is the capital of Samoa?",
        "correct": "Apia",
        "wrong": ["Suva", "Port Vila", "Honiara", "Nuku'alofa"]
    },
    {
        "question": "What is the traditional dress worn by Japanese women called?",
        "correct": "Kimono",
        "wrong": ["Sari", "Hanbok", "Cheongsam", "Barong Tagalog"]
    },
    {
        "question": "What is the only planet in our solar system that rotates clockwise?",
        "correct": "Venus",
        "wrong": [
            "Mars",
            "Jupiter",
            "Earth",
            "Mercury"
        ]
    },
    {
        "question": "How long does it take light from the Sun to reach Earth?",
        "correct": "About 8 minutes",
        "wrong": [
            "5 seconds",
            "1 minute",
            "24 hours",
            "1 week"
        ]
    },
    {
        "question": "What is the most abundant gas in the Earth\u2019s atmosphere?",
        "correct": "Nitrogen \u2013 about 78%",
        "wrong": [
            "Oxygen",
            "Carbon dioxide",
            "Argon",
            "Methane"
        ]
    },
    {
        "question": "What color is the blood of an octopus?",
        "correct": "Blue \u2013 due to copper-based hemocyanin",
        "wrong": [
            "Red",
            "Green",
            "Purple",
            "Yellow"
        ]
    },
    {
        "question": "What vegetable was once used as currency in ancient Egypt?",
        "correct": "Garlic",
        "wrong": [
            "Onion",
            "Carrot",
            "Lettuce",
            "Cucumber"
        ]
    },
    {
        "question": "What is the only edible food that never goes bad?",
        "correct": "Honey",
        "wrong": [
            "Salt",
            "Sugar",
            "Rice",
            "Canned Food"
        ]
    },
    {
        "question": "What popular drink was originally invented as a medicine?",
        "correct": "Coca-Cola",
        "wrong": [
            "Pepsi",
            "Sprite",
            "Red Bull",
            "Gatorade"
        ]
    },
    {
        "question": "What country has the longest coastline in the world?",
        "correct": "Canada",
        "wrong": [
            "Russia",
            "Australia",
            "United States",
            "Brazil"
        ]
    },
    {
        "question": "What is the only sea without any coasts?",
        "correct": "Sargasso Sea",
        "wrong": [
            "Mediterranean Sea",
            "Caribbean Sea",
            "Red Sea",
            "Baltic Sea"
        ]
    },
    {
        "question": "What is the flattest country on Earth?",
        "correct": "Maldives",
        "wrong": [
            "Netherlands",
            "Bangladesh",
            "Denmark",
            "Luxembourg"
        ]
    },
    {
        "question": "What country\u2019s flag features a maple leaf?",
        "correct": "Canada",
        "wrong": [
            "United States",
            "Brazil",
            "Australia",
            "New Zealand"
        ]
    },
    {
        "question": "What is the smallest country in the world by land area?",
        "correct": "Vatican City",
        "wrong": [
            "Monaco",
            "Nauru",
            "Tuvalu",
            "San Marino"
        ]
    },
    {
        "question": "What is the most streamed show on Netflix (as of 2023)?",
        "correct": "Stranger Things",
        "wrong": [
            "The Witcher",
            "Bridgerton",
            "Money Heist",
            "Squid Game"
        ]
    },
    {
        "question": "What is the name of the fictional school in Harry Potter?",
        "correct": "Hogwarts",
        "wrong": [
            "Nevermore",
            "Illvermorny",
            "Beauxbatons",
            "Durmstrang"
        ]
    },
    {
        "question": "What famous superhero works as a journalist named Clark Kent?",
        "correct": "Superman",
        "wrong": [
            "Batman",
            "Spider-Man",
            "Iron Man",
            "Wonder Woman"
        ]
    },
    {
        "question": "What is the highest-grossing movie of all time (adjusted for inflation)?",
        "correct": "Gone with the Wind",
        "wrong": [
            "Avatar",
            "Titanic",
            "Avengers: Endgame",
            "Star Wars: The Force Awakens"
        ]
    },
    {
        "question": "What is the name of the dragon in The Hobbit?",
        "correct": "Smaug",
        "wrong": [
            "Drogon",
            "Norbert",
            "Shenron",
            "Falkor"
        ]
    },
    {
        "question": "How many taste buds does the average human tongue have?",
        "correct": "Around 10,000",
        "wrong": [
            "Around 1,000",
            "Around 5,000",
            "Around 20,000",
            "Around 50,000"
        ]
    },
    {
        "question": "What is the fastest-growing tissue in the human body?",
        "correct": "Hair",
        "wrong": [
            "Nails",
            "Skin",
            "Muscle",
            "Bone"
        ]
    },
    {
        "question": "What is the shortest war in recorded history?",
        "correct": "Anglo-Zanzibar War \u2013 lasted 38-45 minutes",
        "wrong": [
            "World War I",
            "World War II",
            "Korean War",
            "Falklands War"
        ]
    },
    {
        "question": "What is the fear of long words called?",
        "correct": "Hippopotomonstrosesquippedaliophobia",
        "wrong": [
            "Nyctophobia",
            "Claustrophobia",
            "Acrophobia",
            "Anthophobia"
        ]
    },
    {
        "question": "What is the only letter not appearing in the periodic table of elements?",
        "correct": "J",
        "wrong": [
            "Z",
            "Q",
            "X",
            "Y"
        ]
    },
    {
        "question": "What animal holds the record for the longest migration?",
        "correct": "Arctic Tern \u2013 over 70,000 km annually",
        "wrong": [
            "Monarch Butterfly",
            "Gray Whale",
            "Salmon",
            "Caribou"
        ]
    },
    {
        "question": "What is the most common eye color in humans?",
        "correct": "Brown",
        "wrong": [
            "Blue",
            "Green",
            "Hazel",
            "Gray"
        ]
    },
    {
        "question": "What is the tallest tree in the world?",
        "correct": "Hyperion, a coast redwood in California",
        "wrong": [
            "Sequoia",
            "Douglas Fir",
            "Pine",
            "Oak"
        ]
    },
    {
        "question": "What is the largest living organism on Earth?",
        "correct": "A fungus in Oregon \u2013 Armillaria ostoyae",
        "wrong": [
            "Blue Whale",
            "Giant Sequoia",
            "Giant Jellyfish",
            "Coral Reef"
        ]
    },
    {
        "question": "What was the first computer virus ever created?",
        "correct": "Creeper, in 1971",
        "wrong": [
            "ILOVEYOU",
            "Melissa",
            "MyDoom",
            "Code Red"
        ]
    },
    {
        "question": "What company made the first smartphone?",
        "correct": "IBM \u2013 Simon Personal Communicator, 1992",
        "wrong": [
            "Apple",
            "Samsung",
            "Motorola",
            "Nokia"
        ]
    },
    {
        "question": "What year was the first email sent?",
        "correct": "1971",
        "wrong": [
            "1965",
            "1980",
            "1995",
            "2000"
        ]
    },
    {
        "question": "What is the most-used search engine in the world?",
        "correct": "Google",
        "wrong": [
            "Bing",
            "Yahoo",
            "DuckDuckGo",
            "Ask.com"
        ]
    },
    {
        "question": "What was the first social media platform ever created?",
        "correct": "Six Degrees, launched in 1997",
        "wrong": [
            "Facebook",
            "MySpace",
            "Twitter",
            "LinkedIn"
        ]
    }
]